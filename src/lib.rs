...
pub trait GetDataBase {
    /// 返回当前地址空间下，共享区域的首地址。
    ///
    /// 由于crate_interface的限制，无法将该接口声明为unsafe。然而，该接口具有额外的安全要求。
    ///
    /// SAFETY:
    ///
    /// 1. 在同一地址空间中多次调用该函数时，首地址（得到的返回值）需保持不变。
    /// 2. 如果将指针的较低位用作标志位，则需保证首地址按最高标志位对齐。（这是为了保证指针在地址转换前后标志位不变）
    /// 3. 如果架构或硬件不支持非对齐访问，则首地址需要按指针数据类型的大小对齐。（这是为了保证偏移量在不同的地址空间中使用时不会造成不对齐）
